...
function text(body: Record<string, unknown>, key: string, max: number): string {
  ...
  if (typeof value !== 'string' || value.length > max)
    throw new InputError(`${key}: invalid or too long (maximum ${max} characters).`);
  return value.trim();
}
...
export function recommendModules(niche: string, salesNotes: string): DemoModule[] {
  ...
  if (/facebook|lead form|nurtur|follow.up|response speed|slow.*respond/.test(notes))
    return ['nurture', 'chatbot', 'roi'];
  if (/missed call|unanswered|after.hours|receptionist/.test(notes))
    return ['missed_call', 'voice', 'chatbot', 'roi'];
  if (/website|seo|google|ranking/.test(notes)) return ['audit', 'chatbot', 'roi'];
  return /insurance|real estate/i.test(niche)
    ? ['nurture', 'chatbot', 'roi']
    : ['voice', 'missed_call', 'chatbot', 'roi'];
}
export function parseInput(raw: unknown): DemoInput {
  ...
  if (prospectBusinessId && !UUID.test(prospectBusinessId))
    throw new InputError('Invalid prospect ID.');
  ...
  if (body.selectionMode === 'recommended') modules = recommendModules(niche, salesNotes);
    ...
    if (
      !values.length ||
      values.some((item) => typeof item !== 'string' || !Object.hasOwn(MODULES, item))
    ) {
...
export function emptyEvidence(sourceUrl = ''): WebsiteEvidence {
  return {
    ...
    description: '',
    observations: [],
    warning: 'Homepage not checked. Google Business Profile data is not fetched by this release.',
  };
}
...
export function extractEvidence(
  ...
  const description = plain(
    descriptionTag.match(/\bcontent\s*=\s*(["'])([\s\S]*?)\1/i)?.[2] ?? '',
  ).slice(0, 600);
  return {
    status: 'fetched',
    sourceUrl,
    ...
    observations: [
      ...
      description
        ...
        : 'No quoted meta description was detected in the fetched HTML.',
      /name\s*=\s*["']viewport["']/i.test(html)
        ? 'A mobile viewport tag was detected.'
        : 'No quoted mobile viewport tag was detected.',
      /href\s*=\s*["']tel:/i.test(html)
        ? 'A telephone link was detected.'
        : 'No quoted telephone link was detected.',
      /application\/ld\+json/i.test(html)
        ? 'A JSON-LD marker was detected; its validity was not assessed.'
        : 'No JSON-LD marker was detected.',
    ],
    warning:
      'Single-page HTML observations, not a Lighthouse test, accessibility audit, ranking report, or revenue estimate. JavaScript-rendered features may be missed. Verify findings before publishing. Google Business Profile data was not fetched.',
  };
}
function packServices(niche: string): string[] {
  ...
  if (/hvac|heating|cooling/i.test(niche))
    return ['Heating inquiry', 'Cooling inquiry', 'Maintenance consultation'];
...
export function generatePresentation(
  input: DemoInput,
  websiteEvidence = emptyEvidence(input.websiteUrl),
): Presentation {
  const services = [
    ...new Set(
      input.services
  ...
  return {
    ...
    summary:
      input.publicSummary ||
      websiteEvidence.description ||
      `Explore a sample lead-response experience for ${input.businessName}.`,
    modules: [...input.modules],
    websiteEvidence,
    disclosure:
      'Illustrative sales demo created by Steel Scale Systems, not this business’s live support system. No real calls, SMS messages, policy quotes, or appointments are created. Do not enter personal or customer information.',
  };
}
export function calculateRoi(values: number[]): {
  recoveredRevenue: number;
  contributionAfterFee: number;
  breakEvenJobs: number | null;
} {
  if (values.length !== 7 || values.some((n) => !Number.isFinite(n) || n < 0))
    throw new InputError('Enter finite, nonnegative values.');
  const [missed, recovery, qualification, close, jobValue, margin, fee] = values as [
    number,
    number,
    number,
    number,
  ...
  if (
    ...
    jobValue > 10000000 ||
    fee > 10000000 ||
    [recovery, qualification, close, margin].some((n) => n > 100)
  )
    throw new InputError('Percentages must be 0–100; volume/value limits exceeded.');
  const recoveredRevenue =
    missed * (recovery / 100) * (qualification / 100) * (close / 100) * jobValue;
  const jobContribution = (jobValue * margin) / 100;
  return {
    recoveredRevenue,
    contributionAfterFee: (recoveredRevenue * margin) / 100 - fee,
    breakEvenJobs: jobContribution > 0 ? Math.ceil(fee / jobContribution) : null,
  };
}
