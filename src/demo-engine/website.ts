...
async function getHomepage(input: string): Promise<{ html: string; url: string }> {
  ...
  for (let redirects = 0; redirects <= 2; redirects += 1) {
    if (!permittedHostname(url.hostname) || baseHost(url.hostname) !== initialHost)
      throw new Error('Unsafe website host or cross-domain redirect.');
    ...
    if (!addresses.length || addresses.some(({ address }) => !isPublicAddress(address)))
      throw new Error('Website resolves to a non-public address.');
    const address = addresses[0];
    if (!address) throw new Error('No public address.');
    const result = await new Promise<{ status: number; location?: string; html: string }>(
      (resolve, reject) => {
        ...
        const req = transport(
          url,
          {
            ...
            headers: {
              'user-agent': 'SteelScaleDemoResearch/1.0',
              accept: 'text/html',
          ...
          (res) => {
            ...
            if (status >= 300 && status < 400) {
              ...
              res.destroy();
              resolve({ status, ...(location ? { location } : {}), html: '' });
              return;
            }
            if (
              status < 200 ||
              ...
              res.destroy();
              reject(new Error('Website did not return uncompressed HTML.'));
              return;
            }
            if (Number(res.headers['content-length'] ?? 0) > MAX_BYTES) {
              res.destroy();
              reject(new Error('Website response too large.'));
              return;
            }
            const chunks: Buffer[] = [];
            let size = 0;
    ...
    if (result.status >= 300 && result.status < 400) {
      ...
      if (url.protocol === 'https:' && nextUrl.protocol !== 'https:')
        throw new Error('Refusing HTTPS downgrade.');
...
export async function researchHomepage(url: string): Promise<WebsiteEvidence> {
  try {
    ...
    return {
      ...
      warning:
        'Homepage could not be safely fetched. Use operator-entered information; no findings have been invented. Google Business Profile was not fetched.',
    };
  }
}
